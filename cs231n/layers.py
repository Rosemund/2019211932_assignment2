...
def batchnorm_backward(dout, cache):
    ...
    dx_minus_mean_1 = dx_norm / np.sqrt(sample_var + eps)
    #dsqrt_var = - np.sum(dx_norm * (x - sample_mean), axis=0) / (sample_var + eps)
    dsqrt_var = - np.sum(dx_norm * ((x - sample_mean) / (sample_var + eps)), axis=0)
    dvar = 0.5 * dsqrt_var / np.sqrt(sample_var + eps)
    #dx_square = np.ones_like(x) / N * dvar
    #dx_minus_mean_2 = dx_square * (x - sample_mean) * 2
    dx_minus_mean_2 = 2 * np.ones_like(x) * (x - sample_mean) / N * dvar
    dx_minus_mean = dx_minus_mean_1 + dx_minus_mean_2
    dx = dx_minus_mean - np.ones_like(x) / N * np.sum(dx_minus_mean, axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dvar = np.sum((x - sample_mean) * -0.5 * (sample_var + eps)**-1.5 * dx_norm, axis = 0)
    dmean = np.sum(dx_norm / -np.sqrt(sample_var + eps), axis = 0) + dvar * np.sum(-2 * (x - sample_mean) / N, axis = 0)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    x_T_norm = (x.T - sample_mean) / np.sqrt(sample_var + eps)
    x_norm = x_T_norm.T
    out = x_norm * gamma + beta
...
def layernorm_backward(dout, cache):
    ...
    dx_norm = dout.T * gamma[:,np.newaxis]
    dv = np.sum((x.T - sample_mean) * -0.5 * (sample_var + eps)**-1.5 * dx_norm, axis=0)
    dm = np.sum(dx_norm * -1 * (sample_var + eps)**-0.5, axis=0) + np.sum(dv * (x.T - sample_mean) * -2 / N, axis=0)
    dx_T = dx_norm / (sample_var + eps)**0.5 + dv * 2 * (x.T - sample_mean) / N + dm / N
    dx = dx_T.T
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dx_flattened = (1 / np.sqrt(group_var + eps)) * (dx_norm_flattened - (1 / D_) * (np.sum(dx_norm_flattened, axis=1, keepdims=True) + (x_flattened - group_mean) / (group_var + eps) * np.sum(dx_norm_flattened * (x_flattened - group_mean), axis=1, keepdims=True)))
