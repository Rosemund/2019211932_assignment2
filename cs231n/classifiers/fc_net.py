...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        fc_cache, norm_cache, relu_cache, drop_cache = {}, {}, {}, {}
        out = X
        for i in range(self.num_layers-1):
            W, b = self.params['W'+str(i+1)], self.params['b'+str(i+1)]
            out, fc_cache[i] = affine_forward(out, W, b)
            if self.normalization=='batchnorm':
                gamma, beta = self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)]
                out, norm_cache[i] = batchnorm_forward(out, gamma, beta, self.bn_params[i])
            if self.normalization=='layernorm':
                gamma, beta = self.params['gamma' + str(i + 1)], self.params['beta' + str(i + 1)]
                out, norm_cache[i] = layernorm_forward(out, gamma, beta, self.bn_params[i])
        ...
        for i in list(range(self.num_layers))[::-1]:
            ...
            if i == self.num_layers - 1:
                dout, grads['W'+str(i+1)], grads['b'+str(i+1)] = affine_backward(dout, cache)
            else:
                if self.use_dropout:
                    dout = dropout_backward(dout, drop_cache[i])
                dout = relu_backward(dout, relu_cache[i])
                if self.normalization == 'batchnorm':
                    dout, grads['gamma'+str(i+1)], grads['beta'+str(i+1)] = batchnorm_backward(dout, norm_cache[i])
                if self.normalization == 'layernorm':
                    dout, grads['gamma'+str(i+1)], grads['beta'+str(i+1)] = layernorm_backward(dout, norm_cache[i])
                dout, grads['W'+str(i+1)], grads['b'+str(i+1)] = affine_backward(dout, fc_cache[i])
